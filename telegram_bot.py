...
def envoyer(texte, boutons=None):
    ...
    try:
        ...
        print(f">>> Réponse Telegram : {res.json()}")   # ← ajoute cette ligne
...
def construire_message_visiteurs():
    visiteurs = get_visiteurs()
    if not visiteurs:
        return "😴 *Aucun visiteur actif*", []
...
def mettre_a_jour_live():
    ...
    # Ne rien faire si aucun visiteur
    if not visiteurs:
        print(">>> Aucun visiteur — pas d'envoi")
        return
    ...
    if msg_id:
        ...
        epingler(msg_id)
        print(f">>> Message live envoyé : {msg_id}")
...
def traiter(update):
    ...
    if 'message' in update:
        ...
        if texte in ('/start', '/help'):
            ...
            texte_v, boutons = construire_message_visiteurs()
            envoyer(texte_v, boutons)
        elif texte == '/redirect':
            visiteurs = get_visiteurs()
            if not visiteurs:
...
while True:
    try:
        print(f">>> temps écoulé : {time.time() - dernier_live:.1f}s / {INTERVALLE}s")  # ← ajoute
